...
### Custom Actions and Settings ###
        ...
        self.device_pass = 'YOUR_PASSWORD'
        self.romon_secret = 'YOURSECRET'

    # Function to autohide a message
    def hide_msg(self):
        self.msg_label.hide()
        self.confirm_reset = 0
        self.timer.stop()
    # Function to show a message        
    def show_msg(self, color, msg, time):
        self.msg_label.setText(msg)
        self.msg_label.setStyleSheet(f"color:{color}")
        self.msg_label.show()
        self.timer.start(time)
    # Function to reset the form style    
    ...
    def config_device(self, ssid, wireless_pass, account_num, name, package):
        ...
        if ssid != '' and wireless_pass != '' and len(wireless_pass) >= 8 and account_num != '' and name != '' and self.device_select.currentText() != '' and self.package_select.currentText() != '':
            if self.device_select.currentText() == 'RB2011':
                ...
                add name=bridge1
                /interface wireless
                set [ find default-name=wlan1 ] band=2ghz-b/g/n country="united states" disabled=no frequency=auto mode=ap-bridge ssid="{ssid}" \\
                ...
                /user set [ find where name="admin" ] password="{self.device_pass}"
                """
            ...
            if config != "":   
                ...
                def router_thread():
                    try:
                        ...
                        self.send_config(timestamp=timestamp, path=path, version=1, password=self.device_pass)
                        self.send_config(timestamp=timestamp, path = path2, version=2, password=self.device_pass)
                try:
                    self.show_msg('green', "Device is being configured, please wait",35000)
                    t = threading.Thread(target=router_thread)
                    t.start()
                except:
                    self.show_msg('red', "Could not connect to the device",5000)
            else:
                self.show_msg('red',f"Configuring a {self.device_select.currentText()} is not available at this time",5000)
                
        else:
            if len(wireless_pass) < 8 and wireless_pass !="":
                self.password_line.setStyleSheet("border-radius:10px;\nborder: 1px solid rgb(86, 86, 86);\nbackground-color: rgb(255, 0, 0, 50);")
                self.show_msg('red', "Password must be at least 8 characters long",5000)
            else:
                self.show_msg('red', "Please fill out all highlighted fields",5000)
    ...
    def config_bridg(self, account_num, name, package):
        ...
        if account_num != '' and name != '' and self.device_select.currentText() != '' and package != '':
            ...
            if config != "":   
                ...
                try:
                    self.show_msg('green', "Device is being configured, please wait",35000)
                    t = threading.Thread(target=bridge_thread)
                    t.start()
                except:
                    self.show_msg('red', 'Something went wrong',5000)
            else:
                self.show_msg('red',f"Configuring a {self.device_select.currentText()} is not available at this time",5000)
    ...
    def reset_router(self):
        ...
        if self.confirm_reset == 0:
            self.show_msg('red', "Are you sure that you want to factory reset the device? If so, click the reset button again",5000)
            ...
            def reset_thread():          
                try:
                    self.send_config(timestamp=timestamp, path = path, version=3, password=self.device_pass)
                except:
                    self.send_config(timestamp=timestamp, path = path, version=3, password='')
            try:
                self.show_msg('green','Resetting device, please wait',35000)
        ...
        if self.fw_line.text() != '':
            ...
            def fw_thread():
                try:
                    ...
                    self.send_config(timestamp=timestamp, path = self.fw_line.text(), version=4, password=self.device_pass)
                    self.send_config(timestamp=timestamp, path = path, version=3, password=self.device_pass)
            try:
                self.show_msg('green','Upgrading device firmware, please wait',40000)
                t = threading.Thread(target=fw_thread, daemon=True)
                t.start()
            except:
                self.show_msg('red', "Something went wrong",5000)
        else:
            self.show_msg('red', "Please select firmware file",5000)
